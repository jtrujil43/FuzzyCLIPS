...
/******************************************************************
    FuzzyCLIPS: Fuzzy Value Hash Table Functions
 ******************************************************************/
...
CLIPSFuzzyValue *AddFuzzyValue(
  Environment *theEnv,
  struct fuzzy_value *fv)
{
   CLIPSFuzzyValue *fvhn;
   /* Simple implementation: allocate a CLIPSFuzzyValue wrapper */
   fvhn = get_struct(theEnv, clipsFuzzyValue);
   fvhn->header.type = FUZZY_VALUE_TYPE;
   fvhn->contents = fv;
   fvhn->count = 0;
   fvhn->next = NULL;
   fvhn->bucket = 0;
...
void SetFuzzyValueTable(
  Environment *theEnv,
  CLIPSFuzzyValue **table)
{
   SymbolData(theEnv)->FuzzyValueTable = table;
}
